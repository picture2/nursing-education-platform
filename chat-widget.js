    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        async function sendMessage() {
            ...
            try {
                ...
                if (data.success) {
                    // الرسالة أُرسلت بنجاح - سيتم التحديث التلقائي
                }
            } catch (error) {
        ...
        // تتبع آخر رسالة لتجنب إعادة الرسم غير الضرورية
        ...
        async function loadConversation() {
            try {
                ...
                if (data.success && data.data.length > 0) {
                    // التحقق إذا كانت هناك رسائل جديدة
                    const latestMessageId = data.data[data.data.length - 1].id;
                    
                    // إعادة الرسم فقط إذا تغيرت الرسائل
                    if (lastMessageId !== latestMessageId) {
                        ...
                        data.data.forEach(msg => {
                            ...
                            const isAdminMessage = msg.isAdminMessage || msg.userId === 'admin';
                            const sender = isAdminMessage ? 'admin' : 'user';
                            
                            addMessageToUI(msg.message, sender, msg.timestamp);
                            
                            // رد الأدمن القديم (للتوافق مع النظام القديم)
                            if (msg.adminReply && !msg.isAdminMessage) {
        ...
        // التحديث التلقائي كل ثانيتين (سريع وسلس)
        setInterval(() => {
